...
class AccBarPlot extends BarPlot
{
    ...
    public function __construct($plots)
    {
        ...
        if (1 > $this->nbrplots) {
            throw Util\JpGraphError::make(2010); //('Cannot create AccBarPlot from empty plot array.');
        }
        ...
        for ($i = 0; $i < $this->nbrplots; ++$i) {
            ...
            throw Util\JpGraphError::make(2011, $i); //("Acc bar plot element nbr $i is undefined or empty.");
        }
        ...
        for ($i = 0; $i < $this->nbrplots; ++$i) {
            ...
            throw Util\JpGraphError::make(2015);
    ...
    public function Legend($graph)
    {
        ...
        for ($i = $n - 1; 0 <= $i; --$i) {
            ...
            if (!($this->plots[$i] instanceof BarPlot)) {
                throw Util\JpGraphError::make(2012, $c);
    ...
    public function Max()
    {
        ...
        for ($i = 0; $i < $nmax; ++$i) {
            ...
            if (!isset($this->plots[0]->coords[0][$i])) {
                throw Util\JpGraphError::make(2014);
            }
            ...
            for ($j = 0; $j < $this->nbrplots; ++$j) {
                ...
                if ($this->bar_shadow) {
                    ...
                    if ($this->nbrplots - 1 === $j) {
                        ...
                        if (\is_array($this->bar_shadow_color)) {
                            ...
                            if (0 === $numcolors) {
                                throw Util\JpGraphError::make(2013); //('You have specified an empty array for shadow colors in the bar plot.');
                            }
                ...
                if ($this->plots[$j]->grad) {
                    ...
                    if (\is_array($this->plots[$j]->grad_fromcolor)) {
                        ...
                        if (3 === $ng) {
                            if (\is_numeric($this->plots[$j]->grad_fromcolor[0]) && 0 < $this->plots[$j]->grad_fromcolor[0]
                                && 256 > $this->plots[$j]->grad_fromcolor[0]) {
                                // Image\RGB Triple
                ...
                if (-1 < $this->plots[$j]->iPattern) {
                    if (null === $pattern) {
                        $pattern = new Graph\Pattern\RectPatternFactory();
                    }
                ...
                if (Configs::safe_count($this->plots[$j]->csimtargets) > $i) {
